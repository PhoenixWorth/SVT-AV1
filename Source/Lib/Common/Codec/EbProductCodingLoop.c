    ...
    if (block_has_coeff && block_signals_txsize(context_ptr->blk_geom->bsize)) {
        ...
        if (is_inter) {
            ...
            set_mi_row_col(
                ...
                cm->mi_cols);
            MbModeInfo * mbmi = &xd->mi[0]->mbmi;
            xd->above_txfm_context = &context_ptr->txfm_context_array->top_array[txfm_context_above_index];
            xd->left_txfm_context = &context_ptr->txfm_context_array->left_array[txfm_context_left_index];
            mbmi->tx_size = context_ptr->blk_geom->txsize[context_ptr->tx_depth][0];
            mbmi->sb_type = context_ptr->blk_geom->bsize;
            mbmi->use_intrabc = 0;
    ...
    uint64_t                     *y_full_distortion)
{
    ...
    if (context_ptr->md_staging_tx_search == 0)
        ...
        tx_search_skip_fag = picture_control_set_ptr->parent_pcs_ptr->tx_search_level == TX_SEARCH_FULL_LOOP ? EB_FALSE : EB_TRUE;
    ...
    EbAsm                                   asm_type)
{
    ...
    uint8_t  tx_search_skip_fag = (picture_control_set_ptr->parent_pcs_ptr->tx_search_level == TX_SEARCH_INTER_DEPTH && (picture_control_set_ptr->parent_pcs_ptr->atb_mode == 0 || candidate_buffer ->candidate_ptr->type == INTER_MODE)) ? get_skip_tx_search_flag(
        context_ptr->blk_geom->sq_size,
        ref_fast_cost,
        *candidate_buffer->fast_cost_ptr,
